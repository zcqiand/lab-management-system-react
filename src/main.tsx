...
// axios 拦截器（orval 全局 axios，features 层数据获取同走这里）：baseUrl 走
// env（VITE_API_BASE_URL），token 走契约 key lab.accessToken；401 时清持久化
// 落 anonymous（不在此直接跳路由 — FSM 状态变化由 useRequireAuth 守卫消费并重定向）
installHttpClient(
  () => {
    try {
