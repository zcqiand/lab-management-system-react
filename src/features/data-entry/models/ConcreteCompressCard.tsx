 ...
 */
export function ConcreteCompressCard({
  parameter: p,
  record,
  sampleId,
  config,
  ...
  const emit = (nextLoads: number[]) => {
    const { strengths: s, representative: rep } = computeConcreteCompress(
      nextLoads,
      area,
    );
    onChange({
      result: JSON.stringify({ loads: nextLoads, strengths: s, representative: rep }),
    });
  };
