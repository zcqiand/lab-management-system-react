...
const DEFAULT_SUB_PARAMS: CompositeSubConfig[] = [
  {
    ...
    formulaKey: "tensile_strength",
    label: "抗拉强度",
    inputLabel: "最大力 (kN)",
  },
  {
    parameterCode: "IP-0086",
    formulaKey: "yield_strength",
    label: "屈服强度",
    inputLabel: "屈服力 (kN)",
  },
  {
    parameterCode: "IP-0150",
    formulaKey: "passthrough",
    label: "断后伸长率",
    inputLabel: "伸长率 (%)",
  },
  {
    parameterCode: "IP-0097",
    formulaKey: "passthrough",
    label: "最大力总伸长率",
    inputLabel: "最大力总伸长率 (%)",
  },
  {
    parameterCode: "IP-0559",
    formulaKey: "ratio_tensile_over_yield",
    label: "强屈比",
    autoOnly: true,
  },
  {
    parameterCode: "IP-0560",
    formulaKey: "ratio_measured_over_spec_yield",
    label: "超强比",
    autoOnly: true,
  },
];
...
function SectionRow({
  ...
  const strengths = useMemo<number[]>(() => {
    ...
    return state.loads
      .slice(0, count)
      .map((v) => (Number.isFinite(v) && v > 0 ? rounder(v) : 0));
          ...
          {Array.from({ length: count }, (_, i) => {
            ...
            return (
              <tr key={i}>
                ...
                <td className="py-0.5 text-gray-700">{val > 0 ? val.toFixed(2) : "—"}</td>
              </tr>
            );
          })}
        </tbody>
...
export function RebarMechCompositeCard({
  ...
  const handleSave = () => {
    ...
    for (const sub of subs) {
      const state = sections.get(sub.parameterCode) ?? {
        loads: [],
        diameter: 0,
        techReqId: "",
      };
      ...
      if (isStrength) strengths = computeStrengths(state.loads, state.diameter || 0);
        ...
        if (auto) strengths = auto;
          ...
          strengths = state.loads
            .slice(0, count)
            .map((v) => (Number.isFinite(v) && v > 0 ? rounder(v) : 0));
      }
      const mean = meanOf(strengths, rounder);
      const reqOptions = reqByCode.filter(
        (r) => r.inspectionParameterCode === sub.parameterCode,
      );
      const req =
        reqOptions.find((r) => techReqKey(r) === state.techReqId) ?? reqOptions[0];
    ...
    <div className="border rounded p-3 space-y-3">
      ...
      {cfg?.shareDiameter !== false && (
        <div className="flex items-center gap-2 text-xs bg-gray-50 rounded p-2">
          ...
          <input
            ...
            onChange={(e) =>
              setSharedDiameter(e.target.value === "" ? 0 : Number(e.target.value))
            }
            readOnly={readOnly}
            aria-label="共享公称直径"
            className="w-24 border rounded px-2 py-0.5 text-xs read-only:bg-gray-50 read-only:text-gray-500"
      ...
      <div className="grid grid-cols-2 gap-3">
        {subs.map((s) => {
          ...
          const state = sections.get(s.parameterCode) ?? {
            loads: [],
            diameter: 0,
            techReqId: "",
          };
